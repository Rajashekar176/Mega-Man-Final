...
object SparkMovieRecommendation {
  def main(args: Array[String]) {
    ...
    val movies = sc.textFile(new File(movieLensHomeDir, "hospitals.dat").toString).map { line =>
      val fields = line.split("::")
    ...
    println("Got " + numRatings + " ratings from "
      + numUsers + " users on " + numMovies + " Hospitals.")
    ...
    var bestNumIter = -1
    val rank=bestRank
    val lambda=bestLambda
    val numIter=bestNumIter
...
//    println("The best model improves the baseline by " + "%1.2f".format(improvement) + "%.")
    ...
    var i = 1
    println("Hospitals recommended for you:")
