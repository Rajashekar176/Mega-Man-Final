...
object IPApp {
  ...
  val IMAGE_CATEGORIES = List("blood", "cotton", "syringes")
  ...
  def extractDescriptors(sc: SparkContext, images: RDD[(String, String)]): Unit = {
    ...
    val data = images.map {
      case (name, contents) => {
        val image_path = name.split("ile:/")(1)
        val desc = ImageUtils.descriptors(image_path)
  ...
  def createHistogram(sc: SparkContext, images: RDD[(String, String)]): Unit = {
    ...
    val data = images.map {
      case (name, contents) => {
        ...
        val desc = ImageUtils.bowDescriptors(name.split("ile:/")(1), vocabulary)
        val list = ImageUtils.matToString(desc)
        println("-- " + list.size)

        val segments = name.split("ile:/")
        val fileName = segments(1)
        val seg = fileName.split("/")
        val cat = seg(seg.length - 2)
        println(cat)
  ...
  def testImageClassification(sc: SparkContext) = {
    ...
    println("Predicting test image : " + IMAGE_CATEGORIES(p.toInt))

    iOSConnector.sendCommandToRobot(IMAGE_CATEGORIES(p.toInt))
  ...
  def main(args: Array[String]) {
    ...
    val ip = InetAddress.getByName("10.205.1.135").getHostName
